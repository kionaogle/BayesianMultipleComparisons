# Hierarchcial Bayesian model that fits a SAM model 
# to baseline water potential data:
...
model{
  ...
  # Hierachical normal priors for shrub-specific parameters
  ...
  # Root node priors for mu.b and tau.b; pop'n (global) level parameters
  for(k in 1:7){#for each parameter 
    ...
    # params for folded t; set Ab values in datalist
    Bb[k] <- 1/(Ab[k]*Ab[k])
  }
  ...
  # For focal HB model, compute contrasts to evaluate pairwise differences in all 
  # regression parameters among treatment groups:
