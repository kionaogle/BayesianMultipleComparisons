# The Complete Pooling (CP) version of the "allometric scaling" model.
...
model
{
  ...
  for(i in 1:N){
    ...
    # or the "specialized" model, but we set all species-level parameters equal to common,
    # global parmeters.
    Y[i,1:2] ~ dmnorm(mu[i,1:2], Omega[1:2,1:2])
    
    # Define the mean vector (i.e., scaling model that relates the true or latent variables).
    # alpha is normalizing constant, and beta is scaling exponent; while alpha and beta are
    # indexed by species here, the species values will be set equal to the global parameters.
    for(k in 1:2){
