...
# for(i in 1:3){
#   inits[[i]][[1]]<-saved.state.cp[[2]][[i]][[1]][,c(1,3,5,7,9,11,13,15)]
#   inits[[i]][[2]]<-saved.state.cp[[2]][[i]][[2]]
#   inits[[i]][[3]]<-saved.state.cp[[2]][[i]][[3]]
#   inits[[i]][[4]]<-saved.state.cp[[2]][[i]][[4]]
#   inits[[i]][[5]]<-saved.state.cp[[2]][[i]][[5]]
...
# The saved state was never loaded 
load("inits_baseline_cp.Rdata")
### Run NH model
jm_cp <- jags.model("R model baseline CP.R", data=datalist, inits=saved.state.cp[[2]], n.chains=3, n.adapt = updateits)
...
#save model
save(jm_cp, file = "Output/jm_cp_baseline.Rdata")
...
newinits<-initfind(coda_cp)#default is final iteration
newinits[[1]]
#remove non-root node variables
#retain a, deltaA-E, tau
saved.state.cp <- removevars(initsin = newinits, variables=c(5:6, 8:10)) # remove non-variable nodes
#check both items in list
#saved.state.cp[[1]]
saved.state.cp[[2]]#this goes into jags.model() to reinitialize
save(saved.state.cp, file="inits_baseline_cp.Rdata") #for local
#load(file="inits_baseline_cp.Rdata")

#check convergence
#gel.cp<-gelman.diag(coda_cp, multivariate = F)
#gel.cp$psrf[match("deviance", row.names(gel.cp$psrf)),]
#gel.cp$psrf[match("b[1,1]", row.names(gel.cp$psrf)):match("b[7,16]", row.names(gel.cp$psrf)),]
#gel.cp$psrf[match("wA[1]", row.names(gel.cp$psrf)):match("wA[7]", row.names(gel.cp$psrf)),]
#gel.cp$psrf[match("wB[1]", row.names(gel.cp$psrf)):match("wB[7]", row.names(gel.cp$psrf)),]
#gel.cp$psrf[match("wC[1]", row.names(gel.cp$psrf)):match("wC[7]", row.names(gel.cp$psrf)),]
#gel.cp$psrf[match("tau", row.names(gel.cp$psrf)),]
